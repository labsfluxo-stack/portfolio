...
describe('runCommand', () => {
  ...
  it('projects --stack sem nenhuma correspondência devolve lista vazia, sem inventar texto', () => {
    expect(runCommand('projects --stack nestjs', ctx)).toEqual([])
  })
  ...
  it('COMMAND_NAMES (exceto clear) bate exatamente com as chaves de responses, nos dois idiomas', () => {
    const expected = COMMAND_NAMES.filter((name) => name !== 'clear')
      .slice()
      .sort()
    expect(Object.keys(pt.terminal.responses).sort()).toEqual(expected)
    expect(Object.keys(en.terminal.responses).sort()).toEqual(expected)
  })
})
...
describe('Terminal (seção)', () => {
  it('publica uma <dl> estática com todos os comandos e suas respostas, fora de qualquer <script>', () => {
    ...
    for (const [command, lines] of Object.entries(pt.terminal.responses)) {
